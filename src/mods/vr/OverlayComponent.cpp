...
void OverlayComponent::update_input_mouse_emulation() {
    ...
    if (m_framework_intersect_state.intersecting && VR::get()->is_using_controllers()) {
        auto vr = VR::get();
        auto& io = ImGui::GetIO();
        const auto is_initial_frame = vr->syncs_every_frame() || vr->m_engine_frame_count % 2 == vr->m_left_eye_interval;
...
//    memcpy(io.KeysDown, m_initial_imgui_input_state.KeysDown, sizeof(io.KeysDown));
    ...
    const auto rendertarget_width = is_d3d11 ? g_framework->get_overlay_width_d3d11() : g_framework->get_rendertarget_width_d3d12();
    const auto rendertarget_height = is_d3d11 ? g_framework->get_overlay_height_d3d11() : g_framework->get_rendertarget_height_d3d12();
...
bool OverlayComponent::update_wrist_overlay_openvr() {
    ...
    const auto last_window_pos = g_framework->get_last_window_pos();
    const auto last_window_size = g_framework->get_last_window_size();
    const auto render_target_width = is_d3d11 ? g_framework->get_overlay_width_d3d11() : g_framework->get_rendertarget_width_d3d12();
    const auto render_target_height = is_d3d11 ? g_framework->get_overlay_height_d3d11() : g_framework->get_rendertarget_height_d3d12();
    ...
    if (vr->is_using_controllers()) {
        ...
        if (glm::intersectRayPlane<glm::vec3>(start, fwd, plane_pos, glm::normalize(glm::vec3{glm_matrix[2]}), intersection_distance)) {
            ...
            if (local_point.x >= -w_half && local_point.x <= w_half && local_point.y >= -h_half && local_point.y <= h_half) {
                ...
                if (ui_swapchain.handle) {
                    const auto client_x = (int32_t)((float)ui_swapchain.width * x);
                    const auto client_y = (int32_t)((float)ui_swapchain.height * y);
